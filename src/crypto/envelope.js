...
export function isEnvelope(value) {
  ...
  if (version !== ENVELOPE_VERSION || typeof salt !== "string" || typeof iv !== "string" || typeof payload !== "string") {
    return false;
  }
...
export async function deriveBucketKey(password, salt, iterations = PBKDF2_ITERATIONS) {
  ...
  if (!Number.isInteger(iterations) || iterations < PBKDF2_ITERATIONS) {
    throw new Error(`PBKDF2 iterations must be at least ${PBKDF2_ITERATIONS}`);
  }
...
export async function encryptWithKey(key, salt, plaintext) {
  if (!(salt instanceof Uint8Array) || salt.length !== SALT_LENGTH) {
    throw new Error("Invalid encryption salt");
  }
...
export async function createEncryptedBucket(password, plaintext, iterations = PBKDF2_ITERATIONS) {
  ...
  const envelope = await encryptWithKey(key, salt, plaintext);
  return { envelope, key, salt };
}

export async function unlockEncryptedBucket(password, envelope, iterations = PBKDF2_ITERATIONS) {
...
export async function decryptEnvelope(password, envelope, iterations = PBKDF2_ITERATIONS) {
  return (await unlockEncryptedBucket(password, envelope, iterations)).plaintext;
}
